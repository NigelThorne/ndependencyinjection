namespace QuickGraph.Algorithms.Search
{
    ...
    public class EdgeHeightFirstSearchAlgorithm : ITreeEdgeBuilderAlgorithm, IEdgeColorizerAlgorithm, IAlgorithm
    {
        ...
        public EdgeHeightFirstSearchAlgorithm(IBidirectionalVertexAndEdgeListGraph g, EdgeColorDictionary colors)
        {
            this.maxDepth = 0x7fffffff;
            if (g == null)
            {
        ...
        public void Compute(IVertex v)
        {
            ...
            IEdgeEnumerator enumerator = this.VisitedGraph.InEdges(v).GetEnumerator();
        ...
        public void Visit(IEdge se, int depth)
        {
            if (depth <= this.maxDepth)
            {
                ...
                this.OnTreeEdge(se);
                IEdgeEnumerator enumerator = this.VisitedGraph.InEdges(se.get_Target()).GetEnumerator();
                while (enumerator.MoveNext())
                {
                    IEdge e = enumerator.get_Current();
                    if (this.EdgeColors.get_Item(e) == null)
